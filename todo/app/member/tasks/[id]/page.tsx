...
import { UploadCloud, Paperclip, CheckCircle } from 'lucide-react';
import { tasks } from '@/lib/data';

export default async function MemberTaskDetailPage({ params }: { params: Promise<{ id: string }> }) {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
           <div>
             <div className="flex items-center gap-3 mb-2">
               ...
               <span className="text-xs text-red-500 font-bold bg-red-50 px-2 py-1 rounded">Due: {task.dueDate}</span>
             </div>
             ...
             <Card className="border-none shadow-sm shadow-gray-200/50">
               ...
               <CardContent className="space-y-4">
                 <div className="flex justify-between border-b border-gray-100 pb-3">
                   ...
                   <span className="font-semibold text-[#003631] text-sm">{task.assignedBy}</span>
                 </div>
                 <div className="flex justify-between border-b border-gray-100 pb-3">
                   <span className="text-gray-500 text-sm">Assigned Date</span>
                   <span className="font-semibold text-[#003631] text-sm">{task.assignedDate}</span>
                 </div>
                 <div className="flex justify-between border-b border-gray-100 pb-3">
                   <span className="text-gray-500 text-sm">Priority</span>
                   <span className={`text-sm font-bold ${task.priority === 'High' ? 'text-red-500' : 'text-orange-500'}`}>{task.priority}</span>
                 </div>
               </CardContent>
             </Card>
             ...
             <Card className="border-none shadow-sm shadow-gray-200/50 bg-[#FFEDA8]/30">
               <CardHeader>
                 <CardTitle className="text-md flex items-center">
                   <Paperclip className="w-4 h-4 mr-2" /> Attachments ({task.attachments})
                 </CardTitle>
               </CardHeader>
